...
import {
  ButtonStylePropParams,
  MessageStylePropParams,
  MessageStyleProps,
} from './types';
...
export function getMessageStyleProps({
  styleParams,
  layout,
}: MessageStylePropParams): MessageStyleProps {
  // view style applied to the componentWrapper and primary container views
  const { componentWrapper, container } = getContainerAndWrapperStyle({
    styleParams,
    layout,
  });
