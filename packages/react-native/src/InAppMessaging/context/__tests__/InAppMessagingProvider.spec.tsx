...
jest.mock('@aws-amplify/notifications', () => ({
  ...
  ...(jest.requireActual('@aws-amplify/notifications') as any),
  Notifications: { InAppMessaging: { onMessageReceived: jest.fn() } },
}));

const { InAppMessaging } = Notifications;

let onMessageReceivedCallback: InAppMessagingContextType['displayInAppMessage'] =
  null;
...
describe('InAppMessagingProvider', () => {
  ...
  it('vends custom components received as props', () => {
    ...
    expect(
      (consumer.props as InAppMessagingContextType).components.BannerMessage
    ).toBe(BannerMessage);
  });

  it('vends custom style received as props', () => {
    const style = {
      BannerMessage: { container: { backgroundColor: 'lavenderblush' } },
    };
